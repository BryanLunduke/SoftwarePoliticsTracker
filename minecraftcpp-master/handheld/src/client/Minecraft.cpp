...
#else
	// Server discovery/ping-for-hosts isn't supported/reliable on Windows in this build.
	isLookingForMultiplayer = false;
	// Ensure any previous list is cleared and no callback is left active
	raknetInstance->clearServerList();
	if (netCallback) { delete netCallback; netCallback = NULL; }
#endif
}
...
void Minecraft::cancelLocateMultiplayer() {
	...
	netCallback = NULL;
#else
	// Nothing to stop on Windows; just ensure state is clean
	isLookingForMultiplayer = false;
	raknetInstance->clearServerList();
	if (netCallback) { delete netCallback; netCallback = NULL; }
#endif
...
bool Minecraft::connectToHost(const std::string& host, int port) {
 if (netCallback) { delete netCallback; netCallback = NULL; }
 netCallback = new ClientSideNetworkHandler(this, raknetInstance);
 return raknetInstance->connect(host.c_str(), port);
}
