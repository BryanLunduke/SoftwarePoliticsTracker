...
public:
	...
	// Delete key (use VK_DELETE / common code46)
	static const int KEY_DELETE =46;

	// Arrow keys, Home/End (common VK codes)
	...
	static void feed(unsigned char keyCode, int state) {
		// Debug: log incoming key events to help diagnose missing backspace/delete
		LOGW("Keyboard::feed key=%d state=%d", (int)keyCode, state);
		_inputs.push_back(KeyboardAction(keyCode, state));

		_states[keyCode] = state;
	}
	static void feedText(char character) {
		// Debug: log incoming text characters to see what the platform sends
		LOGW("Keyboard::feedText char=%d ('%c')", (int)(unsigned char)character, (character >=32 && character <127) ? character : '?');
